...
def add_pearson(pearson: Pearson):
    ...
    if pearson_id in people:
        raise fastapi.HTTPException(status_code=400, detail="Podana osoba juz istnieje w bazie")
...
def get_by_last_name(last_name: str = Query(None, title="name")):
    ...
    if not result:
        raise fastapi.HTTPException(status_code=404, detail="Brak osoby o podanym nazwisku w bazie")
    else:
        return result
